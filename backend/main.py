...
def root():
    """Root endpoint"""
    return jsonify({"message": "SMART on FHIR Backend API", "version": "1.0.0"})
...
if __name__ == "__main__":
    ...
    if ssl_certfile and ssl_keyfile and os.path.exists(ssl_certfile) and os.path.exists(ssl_keyfile):
        ...
        if not ssl_certfile or not ssl_keyfile:
            print("INFO: Running without local SSL. Hosting services provide SSL automatically.")
        else:
